...
The repository SHALL provide `scripts/validate_sounds.py`, which validates every sound JSON under `data/sounds/` — `sampleRate` consistency (22,050 Hz), positive `sampleCount`, loop-point bounds (`0 ≤ begin < end ≤ count` for every voice with loop points), and per-slot signal stats (RMS, peak, DC offset, silence, clipping) — and SHALL exit with a nonzero code when any file fails.
