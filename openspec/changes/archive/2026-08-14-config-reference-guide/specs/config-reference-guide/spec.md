...
The repository SHALL provide a Klipper-style reference guide at `GUIDE/Config_Reference.md` documenting the hardware config (`hardware-<BOARD>.json`) end-to-end: every config section (`sound`, `drivetrain`, `lights`, `animation`, `telemetry`, `battery`) with each parameter's name, type, default value, allowed values, and a description, where defaults match the firmware's `HardwareConfig` struct defaults.
