...
- **D5: Quirks stay quirks.** The validator must match parser reality: `turn_light.type` is accepted-but-ignored (skip in key diff), `brake_light`/`reversing_light` `brightness_max` is accepted-but-ignored (skip or allow in schema). Motor `type` is derived from the `hardware` token, so the motors accept no `type` key at all (removed from schema + parser allow-list; a stray `type` now warns as an unknown key). Validation documents reality; it doesn't "clean up" the quirks.
