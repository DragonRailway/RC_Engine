...
The system SHALL determine the connected LiPo cell count from `hardware-config.json` (`battery.cell_count`, 1–4 cells) and establish a low-voltage cutoff threshold equal to the configured `battery.cutoff_voltage` (default 3.3V) per cell.

#### Scenario: Single-cell (1S) pack configured
- **WHEN** `hardware-config.json` declares `battery.cell_count: 1`
...
- **WHEN** `hardware-config.json` declares `battery.cell_count: N` (2–4)
...
When `hardware-config.json` omits `battery.cell_count` (or declares 0), the system SHALL auto-detect the connected LiPo cell count at startup based on measured battery voltage and establish a low-voltage cutoff threshold equal to the configured `cutoff_voltage` (default 3.3V) per cell.
